...
// when you want 2 rows, and height 150px for both of them
...
// when you want 3 columns, and width 150px for both of them
// describe the width of the columns

grid-template-columns: 150px 150px 150px;
grid-template-columns: repeat(3, 150px); // will create 3 columns/ tracks with 150px width each
grid-template-columns: repeat(2, 150px) 300px;  
// will create 2 columns/ tracks with 150px width each and third column track with 300px width
...
// will create 2 columns/ tracks with 150px width each and third column track will occupy all the available space
...
## SHORTHAND FOR grid-row/column-start/end is grid-row/column

OR => grid-row: 2 / 3; (row start / row end)
grid-column: 2 / 3; (column start / column end)

## SHORTHAND FOR grid-row/column is grid-area
...
OR => grid-column: 1 / -1;  
// if you don't know how many columns to span, all you know is till end then '-1' will do the job
...
- The automatic placement algorithm, it tries to keep the grid items that we have in order, so it tries to follow that order.
  ...
  but if we want to fix that, we can actually do it by :\_\*
...
- The CSS Grid specification says that a fractional unit fills up the entire remaining space but it is never smaller
